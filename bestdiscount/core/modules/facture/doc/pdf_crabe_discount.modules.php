...
class pdf_crabe_discount extends ModelePDFFactures
{
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->dir_output) {
			...
			if (file_exists($dir)) {
				...
				// Loop on each lines
				for ($i = 0; $i < $nblines; $i++) {
					if($object->lines[$i]->total_ht < 0) {
						$discount = new DiscountAbsolute($this->db);
						$discount->fetch($object->lines[$i]->fk_remise_except);
						if($discount->fk_facture_source > 0) {
							$acompte_tab[] = $object->lines[$i];
							continue;
						}
					}
				...
				// Display total area
				$posy = $this->_tableau_tot($pdf, $object, $deja_regle, $bottomlasttab, $outputlangs, $acompte_tab);
	 ...
	 */
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs, $acompte_tab)
	{
		// phpcs:enable
		global $conf, $mysoc;
		...
		//Gestion des acomptes en hors taxes
		foreach($acompte_tab as $acompte) {
			$total_ht += $acompte->total_ht*-1;
		}
		...
		$totalttcwithacompte = $total_ttc;
		//Gestion des acomptes en TTC
		foreach($acompte_tab as $acompte) {
			$total_ttc += $acompte->total_ttc*-1;
		}
		...
		$resteapayer = price2num($totalttcwithacompte - $deja_regle - $creditnoteamount - $depositsamount, 'MT');
		if (!empty($object->paye)) {
			$resteapayer = 0;
		}

		if (($deja_regle > 0 || $creditnoteamount > 0 || $depositsamount > 0 || $totalttcwithacompte < $total_ttc) && empty($conf->global->INVOICE_NO_PAYMENT_DETAILS)) {
			...
			foreach($acompte_tab as $acompte) {
				$discount = new DiscountAbsolute($this->db);
				$discount->fetch($acompte->fk_remise_except);
				$sourceref = !empty($discount->discount_type) ? $discount->ref_invoive_supplier_source : $discount->ref_facture_source;
				$libelleproduitservice = $outputlangs->transnoentitiesnoconv("Deposit")." ".$sourceref;
				$index++;
				$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($col2x - $col1x, $tab2_hl, $libelleproduitservice, 0, 'L', 0);
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, price($acompte->total_ttc, 0, $outputlangs), 0, 'R', 0);
			}
